...
function AllCars(){
    ...
    const [filterURL, setFilterURL] = useState('http://localhost:3500/api/car/filter?q=all')
    ...
    useEffect(() => {
        ...
        //     try{
        //         const response = await fetch('http://localhost:3500/api/car')
    ...
    const handleSetFilter = (filter) => {
        setFilterURL(`http://localhost:3500/api/car/filter?q=${filter}`)
        console.log(filterURL)
    ...
    const handleSearchQuery = (search) => {
        setFilterURL(`http://localhost:3500/api/car/search?q=${search}`)
    }
