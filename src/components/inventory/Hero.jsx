...
function Hero(){
    ...
    return(
       <div class="bg-[url('src/assets/blake-meyer-CRNbHjNaljo-unsplash.jpg')] bg-cover bg-center h-130 w-full">
            <div className="w-full h-full text-white flex flex-col items-center justify-center text-center bg-black/15">
                ...
                <input onChange={(e) => setQuery(`http://localhost:3500/api/car/search?q=${e.target.value}`)} className="max-w-2xl bg-white/30 text-sm w-[80dvw]  h-12 m-8 rounded-full pl-4 backdrop-blur-sm border border-white/80" type="text" placeholder="Search by brand, model, or category..."/>  
                <div className="items-center flex flex-col w-[80%] max-w-2xl justify-center md:flex-row gap-1">
                    <button className="mb-3 max-w-lg w-[100%] md:w-[50%] cursor-pointer transition-all ease-in-out p-1.5 text-sm bg-white/30 rounded-md backdrop-blur-sm border border-white/80">{ searchResults.length ? (<><i className="fa-solid fa-sliders mr-3"></i>Search {searchResults.length} cars</>) : (<><i className="fa-solid fa-magnifying-glass mr-3"></i>Search</>)}</button>
